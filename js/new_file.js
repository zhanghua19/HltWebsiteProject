    ...
    var normal_show_fun=function(){
        ...
        $('#J_sidebar .side-oper li.side-user').hover(function(){
            if (YANGTATA.uid == '') {
                ...
                $(this).find('#side-login .user-box p.txt').html('<a target="_blank" href="'+YANGTATA.userurl+'">你好！'+YANGTATA.username+'</a>');
                var _partten = /.*?\/default(\_60)?.jpg$/;
                if ( !_partten.test(YANGTATA.face) ) {
                    $(this).find('#side-login .user-box .pic img').attr('src', YANGTATA.face);
                }
            }
        })
